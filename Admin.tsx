...
const Admin = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const { data: totalOrders } = await supabase.rpc("get_total_orders");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Collect images from both file uploads, existing uploaded images, and manually entered URLs
      let imageUrls: string[] = [];

      // Include existing uploaded images (from uploadedImages state)
      ...
      // Add manually entered URLs, excluding empty strings
  ...
  const handleUpdateOrderStatus = async (
    ...
    try {
      const { error } = await supabase
        .from("orders") // Fixed: Changed from 'products' to 'orders'
